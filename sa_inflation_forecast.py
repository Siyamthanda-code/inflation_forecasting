...
# We keep the first occurrence of any duplicate date.
...
# Now that duplicates are gone, this will work
...
train = df_long[df_long.index < train_cutoff]['Inflation_Rate']
test = df_long[df_long.index >= train_cutoff]['Inflation_Rate']
...
try:
    ...
    y_pred = pd.Series([train.iloc[-1]] * len(test), index=test.index)
    model_success = False
...
# Plot History
plt.plot(train.index, train, label='Historical Data (1975-2024)', color='gray', alpha=0.6)

# Plot Test (Ground Truth from dataset)
plt.plot(test.index, test, label='Economist Projection (Ground Truth)', color='green', marker='o', linestyle='--')

# Plot Forecast
plt.plot(y_pred.index, y_pred, label='Algorithm Forecast', color='red', marker='x', linestyle='--')

plt.title('South Africa Inflation Forecast: Algorithm vs Economist (2025-2026)', fontsize=16)
plt.xlabel('Year')
plt.ylabel('Inflation Rate (%)')
plt.legend()
plt.grid(True, alpha=0.3)

# Highlight the prediction window
plt.axvspan(test.index[0], test.index[-1], color='yellow', alpha=0.1)

plt.tight_layout()
plt.savefig('inflation_forecast_vs_actual.png', dpi=300)
plt.show()

print("\n✅ Analysis Complete. Check 'inflation_forecast_vs_actual.png'.")